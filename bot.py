...
# ============================================================
# فەنکشنی بۆتی هێرشکردن (security) - بە indentation ی ڕاست
# ============================================================
...
# ============================================================
# بۆتی ئیندێکسی دروستکراو (index) - هەموو فەنکشنەکان بە یەک شێواز
# ============================================================
def run_new_bot(token, owner_id, data_dir):
    ...
    def get_locale(lang_code=None):
        ...
        # هەموو دەقەکان لێرە دادەنرێن (بە هەمان شێوەی کۆدی سەرەتایی)
        return {
            "ku": {
    ...
    def ask_broadcast(call):
        ...
        bot.edit_message_text(get_locale().get("ask_broadcast_msg", "نامەکە بنێرە"), chat_id=call.message.chat.id, message_id=call.message.message_id)
    ...
    def ask_forward(call):
        ...
        bot.edit_message_text(get_locale().get("ask_forward_msg", "نامەکە بنێرە"), chat_id=call.message.chat.id, message_id=call.message.message_id)
    ...
    def ask_add_channel(call):
        ...
        bot.edit_message_text(get_locale().get("ask_channel_id", "ئایدی کەناڵ بنێرە"), chat_id=call.message.chat.id, message_id=call.message.message_id)
    ...
    def show_channels_to_delete(call):
        ...
        bot.edit_message_text("کەناڵ هەڵبژێرە:", chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=kb)
    ...
    def toggle_notify(call):
        if not is_admin(call.from_user.id): return
        set_setting(notify_file, "ON" if call.data == "ons" else "OFF")
        bot.answer_callback_query(call.id, "گۆڕدرا.", show_alert=True)
        admin_panel(call.message)

    @bot.callback_query_handler(func=lambda call: call.data in ["obot", "ofbot"])
    def toggle_bot_status(call):
        if not is_admin(call.from_user.id): return
        set_setting(status_file, "ON" if call.data == "obot" else "OFF")
        bot.answer_callback_query(call.id, "گۆڕدرا.", show_alert=True)
        admin_panel(call.message)
    ...
    def ask_ban_unban(call):
        ...
        prompt = get_locale()["ask_ban_id"] if action == "ban" else get_locale()["ask_unban_id"]
    ...
    def ask_admin_id(call):
        ...
        set_state(call.from_user.id, f"await_{action}_id")
        prompt = get_locale()["ask_add_admin_id"] if action == "add_admin" else get_locale()["ask_rem_admin_id"]
        bot.edit_message_text(prompt, chat_id=call.message.chat.id, message_id=call.message.message_id)

    @bot.callback_query_handler(func=lambda call: call.data in ["set_paid", "set_free"])
    def toggle_paid_mode(call):
        if not is_admin(call.from_user.id): return
        set_setting(paid_mode_file, "ON" if call.data == "set_paid" else "OFF")
        bot.answer_callback_query(call.id, "گۆڕدرا.", show_alert=True)
        admin_panel(call.message)
    ...
    def ask_paid_id(call):
        ...
        set_state(call.from_user.id, f"await_{action}_id")
        prompt = get_locale()["ask_add_paid_id"] if action == "add_paid" else get_locale()["ask_rem_paid_id"]
        bot.edit_message_text(prompt, chat_id=call.message.chat.id, message_id=call.message.message_id)

    @bot.callback_query_handler(func=lambda call: call.data == "setup_stars_payment")
    def stars_info(call):
        if not is_admin(call.from_user.id): return
        # show_stars_setup_info پێویستە پێناسە بکرێت، بەڵام بۆ کورتکردنەوە لێرە تەنها پەیامێک دەنێرین
        bot.answer_callback_query(call.id, "ڕێنماییەکە لە کۆدی ڕەسەندا هەیە.", show_alert=True)

    @bot.callback_query_handler(func=lambda call: call.data == "manage_payment_methods")
    def payment_menu(call):
        if not is_admin(call.from_user.id): return
        # payment_management_panel لە کۆدی ڕەسەندا هەیە
        bot.answer_callback_query(call.id, "بەشی پارەدان.", show_alert=True)

    @bot.callback_query_handler(func=lambda call: call.data == "manage_buttons")
    def buttons_menu(call):
        if not is_admin(call.from_user.id): return
        # buttons_management_panel
        bot.answer_callback_query(call.id, "بەشی دوگمەکان.", show_alert=True)

    @bot.callback_query_handler(func=lambda call: call.data == "change_language")
    def change_lang(call):
        if not is_admin(call.from_user.id): return
        # language_panel
        bot.answer_callback_query(call.id, "گۆڕینی زمان.", show_alert=True)

    @bot.callback_query_handler(func=lambda call: call.data == "download_data")
    def download_menu(call):
        if not is_admin(call.from_user.id): return
        # download_data_panel
        bot.answer_callback_query(call.id, "داگرتنی داتا.", show_alert=True)

    @bot.callback_query_handler(func=lambda call: call.data == "set_start_msg")
    def edit_start(call):
        if not is_admin(call.from_user.id): return
        set_state(call.from_user.id, "await_start_msg")
        bot.edit_message_text(get_locale()["set_start_msg_prompt"], chat_id=call.message.chat.id, message_id=call.message.message_id)
    ...
    def handle_state(msg):
        ...
        if state == "await_broadcast":
            ...
            add_line(banned_file, msg.text.strip())
            bot.send_message(msg.chat.id, "بەربەست کرا.")
        elif state == "await_unban_id":
            remove_line(banned_file, msg.text.strip())
            bot.send_message(msg.chat.id, "بەربەست لادرا.")
        elif state == "await_add_admin_id":
            add_line(admins_file, msg.text.strip())
            bot.send_message(msg.chat.id, "ئەدمین زیاد کرا.")
        elif state == "await_rem_admin_id":
            remove_line(admins_file, msg.text.strip())
            bot.send_message(msg.chat.id, "ئەدمین لادرا.")
        elif state == "await_add_paid_id":
            add_line(paid_users_file, msg.text.strip())
            bot.send_message(msg.chat.id, "بەشداربووی پارەیی زیاد کرا.")
        elif state == "await_rem_paid_id":
            remove_line(paid_users_file, msg.text.strip())
            bot.send_message(msg.chat.id, "بەشداربووی پارەیی لادرا.")
        elif state == "await_start_msg":
            set_setting(start_message_file, msg.text)
    ...
    def check_sub(call):
        sub, _ = is_user_subscribed(call.from_user.id)
        if sub:
            bot.answer_callback_query(call.id, "بەخێربێیت!")
